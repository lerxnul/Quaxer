...
local fonts = {}; do
    ...
    local function sanitize_value(v)
        local t = typeof(v)
        if t == "Color3" then
            return {__color = v:ToHex()}
        elseif t == "EnumItem" then
            return {__enum = tostring(v)}
        elseif t == "Vector3" then
            return {__vector3 = {v.X, v.Y, v.Z}}
        elseif t == "Vector2" then
            return {__vector2 = {v.X, v.Y}}
        elseif t == "CFrame" then
            local comps = {v:GetComponents()}
            return {__cframe = comps}
        elseif type(v) == "table" then
            if v.key then
                return {active = v.active, mode = v.mode, key = tostring(v.key)}
            elseif v["Transparency"] and v["Color"] then
                return {Transparency = v["Transparency"], Color = (typeof(v["Color"]) == "Color3") and v["Color"]:ToHex() or tostring(v["Color"])}
            end
            local newTbl = {}
            for k2, v2 in next, v do
                newTbl[k2] = sanitize_value(v2)
            end
            return newTbl
        elseif type(v) == "boolean" or type(v) == "number" or type(v) == "string" or v == nil then
            return v
        else
            return tostring(v)
        end
        ...
        pcall(function()
            if library["items"] then
                for _, child in pairs(library["items"]:GetChildren()) do
                    if child:IsA("Frame") and child.Name == "\0" then
                        local pos = child.Position
                        local size = child.Size
                        Config["__window_position"] = sanitize_value({
                            X = {Scale = pos.X.Scale, Offset = pos.X.Offset},
                            Y = {Scale = pos.Y.Scale, Offset = pos.Y.Offset}
                        })
                        Config["__window_size"] = sanitize_value({
    ...
    local function desanitize_value(v)
        if type(v) ~= "table" then return v end
        if v.__color then
            return hex(v.__color)
        elseif v.__enum then
            return library:convert_enum(v.__enum)
        elseif v.__vector3 then
            return Vector3.new(unpack(v.__vector3))
        elseif v.__vector2 then
            return Vector2.new(unpack(v.__vector2))
        elseif v.__cframe then
            return CFrame.new(unpack(v.__cframe))
        end
        local newTbl = {}
        for k2, v2 in next, v do
            newTbl[k2] = desanitize_value(v2)
        end
        return newTbl
    end
    ...
    function library:load_config(config_json) 
        ...
        if config["__window_position"] and config["__window_size"] then
            pcall(function()
                if library["items"] then
                    for _, child in pairs(library["items"]:GetChildren()) do
                        if child:IsA("Frame") and child.Name == "\0" then
                            local posData = desanitize_value(config["__window_position"])
                            local sizeData = desanitize_value(config["__window_size"])
                            if posData and sizeData and posData.X and posData.Y and sizeData.X and sizeData.Y then
        ...
        for key, v in next, config do 
            ...
            if function_set then 
                local valueToSet = desanitize_value(v)
                if type(valueToSet) == "table" and valueToSet["Transparency"] and valueToSet["Color"] then
                    local col = valueToSet["Color"]
                    col = desanitize_value(col)
                    if type(col) == "string" then
                        local ok, c = pcall(function()
                            return Color3.fromHex(col:gsub("#",""))
                        end)
                        if ok and c then col = c end
                    elseif type(col) == "table" and col.__color then
                        local ok, c = pcall(function()
                            return Color3.fromHex(tostring(col.__color):gsub("#",""))
                        end)
                        if ok and c then col = c end
                    end
                    function_set(col, valueToSet["Transparency"])
                elseif type(valueToSet) == "table" and valueToSet["active"] ~= nil then 
    ...
    function library:update_theme(theme, color)
        ...
        for propertyName, instances in pairs(themes.utility[theme]) do
            for _, instance in pairs(instances) do
                pcall(function()
                    if instance[propertyName] == previous then
        ...
        if theme == "accent" and library["mobile_icon"] then
            local c = color
            local brightness = (c.R * 0.299) + (c.G * 0.587) + (c.B * 0.114)
            library["mobile_icon"].TextColor3 = brightness > 0.6 and rgb(72, 72, 73) or rgb(255, 255, 255)
        end
    end 
    ...
    function library:window(properties)
        local cfg = { 
            suffix = properties.suffix or properties.Suffix or "Beta";
            name = properties.name or properties.Name or "Quaxer";
            game_name = properties.gameInfo or properties.game_info or properties.GameInfo or "Version B1.0 | discord.gg/yhrUTfWMrj";
            size = properties.size or properties.Size or ((uis.TouchEnabled and not uis.MouseEnabled) and dim2(0, 300, 0, 300) or dim2(0, 700, 0, 565));
            selected_tab;
            items = {};
        ...
        if uis.TouchEnabled and not uis.MouseEnabled then
            ...
            library["mobile_toggle"] = mobile_toggle
            library["mobile_icon"] = mobile_icon

            do
                local c = themes.preset.accent
                local brightness = (c.R * 0.299) + (c.G * 0.587) + (c.B * 0.114)
                mobile_icon.TextColor3 = brightness > 0.6 and rgb(72, 72, 73) or rgb(255, 255, 255)
            end
    ...
    function library:dropdown(options) 
        ...
        local items = cfg.items; do 
                ...
                items[ "scrolling_frame" ] = library:create( "ScrollingFrame" , {
                    ...
                    ScrollBarImageColor3 = rgb(72, 72, 73);
        ...
        library:connection(uis.InputChanged, function(input)
            if input.UserInputType == Enum.UserInputType.MouseWheel then
                ...
                if sf_local and cfg.open and library:mouse_in_frame(items[ "dropdown_holder" ]) then
                    local delta = (input.Position and input.Position.Z) or 0
                    local current_y = sf_local.CanvasPosition.Y
                    local max_y = math.max(0, sf_local.CanvasSize.Y.Offset - sf_local.AbsoluteSize.Y)
                    local new_y = clamp(current_y - delta * 30, 0, max_y)
                    sf_local.CanvasPosition = Vector2.new(0, new_y)
                end
            end
        end)
    ...
    function library:init_config(window) 
        ...
        local function resolve_config_name()
            local from_text = flags["config_name_text"]
            if type(from_text) == "string" and from_text ~= "" then
                return from_text
            end
        ...
        section:button({name = "Save", callback = function()
            ...
            if not cfgName or cfgName == "" then
                notifications:create_notification({name = "Quaxer", info = "Config name is empty"})
                return
            end
            writefile(library.directory .. "/configs/" .. cfgName .. ".cfg", library:get_config())
            library:update_config_list()
            notifications:create_notification({name = "Quaxer", info = "Saved config to:\n" .. cfgName})
        end}) 
        section:button({name = "Load", callback = function()
            local cfgName = flags["config_name_list"]
            if not cfgName or cfgName == "" then
                notifications:create_notification({name = "Quaxer", info = "No config selected to load"})
                return
            end
            library:load_config(readfile(library.directory .. "/configs/" .. cfgName .. ".cfg"))  
            library:update_config_list() 
            notifications:create_notification({name = "Quaxer", info = "Loaded config:\n" .. cfgName})
        end})
        section:button({name = "Delete", callback = function()
            local cfgName = flags["config_name_list"]
            if not cfgName or cfgName == "" then
                notifications:create_notification({name = "Quaxer", info = "No config selected to delete"})
                return
            end
            delfile(library.directory .. "/configs/" .. cfgName .. ".cfg")  
            library:update_config_list() 
            notifications:create_notification({name = "Quaxer", info = "Deleted config:\n" .. cfgName})
        end})
